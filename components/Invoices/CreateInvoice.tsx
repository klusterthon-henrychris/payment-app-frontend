...
const CreateInvoice = () => {
  ...
  useEffect(() => {
    async function getSearchProducts() {
      ...
      if (data && data?.data) {
        const initialQuantities = data.data.items.reduce(
          (acc: { [productId: string]: number }, item: CatalogueItem) => {
            acc[item.id] = 0;
            return acc;
          },
          {}
    ...
    if (selectedItems[productId]) {
      setSelectedItems((prevSelectedItems) => ({
        ...
        [productId]: {
          ...
          quantity: Math.max(
            0,
            prevSelectedItems[productId].quantity + increment
          ),
        },
      }));
    }
  ...
  return (
    <div className="flex">
      <button
        className="ml-auto bg-[#008678] text-[#fff] rounded-[8px] p-3 mt-[24px] mr-[24px]"
        onClick={toggleModel}
      >
      ...
      {isModelOpen && (
        ...
        >
          {modelContent === 1 && (
            <figure className="w-[428px] h-[602px] bg-[#fff] rounded-[8px] mt-[96px] mx-auto relative overflow-y-auto">
              {warning.length > 0 && (
              ...
              <div className="flex flex-row justify-between">
                <p className="text-[16px] font-Satoshi text-[#000] font-bold ml-[24px] mt-[24px]">
                  Generate invoice
                </p>
              ...
              <div className="flex flex-row items-center ml-[24px] mt-[24px]">
                <p className="text-neutral-black font-semibold text-sm mt-2 mb-3">
                  Select Item(s)
                </p>
            ...
            <div className="relative w-full">
              ...
              {products.length > 0 &&
                products.map((item, index) => (
                  <div
                    className="flex flex-row items-center justify-between mt-[24px]"
                    key={index}
                  >
                    <div className="flex flex-row items-center ml-[24px] gap-[8px]">
                        ...
                        onChange={(e) => {
                          ...
                          setSelectedItems((prevSelectedItems) => {
                            if (isChecked) {
                              ...
                              const { [item.id]: removedItem, ...rest } =
                                prevSelectedItems;
                              return rest;
                            }
                          });
                      ...
                      <div className="flex flex-col items-center">
                        <p className="font-Satoshi text-[14px] text-[#1E1E1E] font-normal">
                          {item.name}
                        </p>
                        <p className="font-Satoshi text-[14px] text-[#1E1E1E] font-medium">
                          ₦{item.price}
                        </p>
                      </div>
                    </div>
                    <div className="flex flex-row items-center mr-[24px] gap-[8px]">
                      ...
                      <input
                        ...
                        className="w-[56px] h-[40px] py-6 font-Satoshi text-[#9A9A9A] border border-[1px] border-[#D9D9D9] rounded-[8px] outline-none"
                        required
                      />
                      <button
                        onClick={() => updateQuantity(item.id, 1)}
              ...
              <div className="flex flex-row justify-between mt-[24px]">
                <button
                  className="ml-[24px] absolute bottom-[24px] text-[#F44336] font-Satoshi text-[14px] font-medium"
                  onClick={closeModel}
                >
                  Cancel
                </button>
                <button
                  className="absolute bottom-[24px] right-[24px] w-20 h-10 rounded-[8px] p-2.5 bg-[#008678] text-[#fff] font-Satoshi text-[14px] font-medium"
                  onClick={handleNextPageClick}
                  disabled={modelContent === 1 && amount === 0}
                >
                  Next
                </button>
              </div>
